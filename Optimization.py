...
class impurity_binned_2d():
    ...
    def plot(self, i=0, ax=None):
        ...
        bins = self.info[['add_bin1']].reset_index()
        y_min = ax.get_ylim()[1]
        for k,n in bins.values[1:i+1,:]:
            ax.axvline(n, ls='--', lw=0.8, c='k')
            ax.annotate(s='{:,.3g} ({:,.0f})'.format(n,k), 
        ...
        bins = self.info[['add_bin2']].reset_index()
        x_min = ax.get_xlim()[1]   
        for k,n in bins.values[1:i+1,:]:
            ax.axhline(n, ls='--', lw=0.8, c='k')
            ax.annotate(s='{:,.3g} ({:,.0f})'.format(n,k), 
        ...
        # Plot initial `bin1`.
        bins = self.bins_[0].bin_edges1[1:-1]
        if len(bins)>0:
            for n in bins:
                ax.axvline(n, ls='-', lw=1.4, c='r')
                ax.annotate(s='{:,.3g} ({:,.0f})'.format(n,0), 
                            xy=(n, y_min), xytext=(0,10), 
        ...
        # Plot initial `bin2`.
        bins = self.bins_[0].bin_edges2[1:-1]
        if len(bins)>0:
            for n in bins:
                ax.axhline(n, ls='-', lw=1.4, c='r')
                ax.annotate(s='{:,.3g} ({:,.0f})'.format(n,0), 
                            xy=(x_min,n), xytext=(10,0), 
                            ha='left', va='center', c='r', 
                            textcoords='offset points')
